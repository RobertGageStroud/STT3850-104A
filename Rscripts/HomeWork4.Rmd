...
EX
sims <- 10000
samp_mean <- numeric(sims)
for(i in 1:sims){
RS <- rexp(n = 30, rate = 1/10)
samp_mean[i] <- mean(RS)
}
ggplot(data = data.frame(x = samp_mean), aes(x = x, fill = x >= 12)) + geom_vline(xintercept =  12, linetype = "dashed") + scale_fill_manual(values = c("lightblue", "purple"))
```
...
```{r}
PR12 <- mean(samp_mean >= 12)
PR12
RoundedPR12 <- round(PR12 * 100, 2)
```
`r round(PR12 * 100, 2)`%
```{r}
# part c.
